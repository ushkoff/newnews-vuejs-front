...
export default {
  ...
  computed: {
    ...mapGetters(['userLoggedIn', 'userData', 'article']),
  ...
  methods: {
    ...mapActions(['fetchUserData', 'fetchArticle', 'editArticle']),
    ...
    async loadArticleData () {
      ...
      await this.fetchArticle(data).then(() => {            
        ...
        this.textTitleNews = this.article.title
        this.textContentNews = this.article.content
        this.textContentRefs = this.article.refs ?? ''
        this.chooseCategory = this.article.categoryID
        this.country = this.article.country
        this.countryCode = this.article.countryCode
        this.isEditable = this.article.isEditable

        // Security
        if (this.userID !== this.authorID || !this.isEditable) 
    ...
    async submitHandler (recaptchaToken) {
      ...
      await this.editArticle(data).then(() => {
    ...
    quillSettings() {
        ...
        // Retrieve content from API (in html)
        quill.root.innerHTML = this.textContentNews

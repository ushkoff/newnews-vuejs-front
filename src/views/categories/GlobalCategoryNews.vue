      ...
      .row
        div(class="news-container col-12 col-xl-8")
          ...
          .main-news-side(v-else class="pt-2 pb-2 pl-2 pr-2")
            router-link(
              :to="{ name: 'local-news-by-category', params: { slug: this.categorySlugString } }"
              class="go-to-local-cat-link ml-2 mt-4 mb-2"
            ) Go to "{{ catTitle }}" category in Local News <i class="fas fa-angle-double-right"></i>
            div(class="d-flex align-items-center justify-content-start mb-2")
              RefreshBtn(addClass="ml-2" @click="refreshNews")

            div(v-if="globalCategoryNews.length > 0")
              ShortNewsBlock(
                v-for="article in globalCategoryNews"
                :key="article.id"
                :id="article.id"
            ...
            div(v-else)
              ...
              ) Sorry, there is no global news in such category.
...
export default {
  ...
  metaInfo () {
    return { title: this.$title(this.catTitle + ' | Global News') }
  },
  data: () => ({
  ...
  computed: {
    ...mapGetters(['userLoggedIn', 'userData', 'globalCategoryNews', 'globalCategoryNewsFetchedNumber']),
    categorySlugString: {
  ...
  methods: {
    ...mapActions(['fetchUserData', 'fetchGlobalNewsByCategory']),
    ...mapMutations(['clearGlobalCategoryNews']),
    ...
    async loadFirstNews () {
      ...
      await this.fetchGlobalNewsByCategory(data).then(() => {
        this.firstNewsLoading = false
        if (this.globalCategoryNewsFetchedNumber < this.articlesPerPage) this.nextPageIsEmpty = true
      })
    },
  ...
  beforeRouteUpdate (to, from, next) {
    if (to.name === 'global-news-by-category') {
      next()

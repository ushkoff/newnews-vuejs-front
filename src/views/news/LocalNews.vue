      ...
      .row
        div(class="news-container col-12 col-xl-8")
          ...
          .main-news-side(v-else class="pt-2 pb-2 pl-2 pr-2 mb-5")
            ...
            //- LOCAL NEWS list
            div(v-if="localNews.length > 0")
              ShortNewsBlock(
                v-for="article in localNews"
                :key="article.id"
                :id="article.id"
            ...
            div(v-else)
              ...
              span.search-results(
                ...
                class="text-break pl-2 d-block mb-2"
              ) Sorry, there is no news in your country.
              span.search-results(
...
export default {
  name: 'local-news',
  metaInfo () {
    return { title: this.$title('Local News | ' + this.userCountry) }
  },
  data: () => ({
  ...
  computed: {
    ...mapGetters(['userLoggedIn', 'userData', 'localNews', 'localNewsFetchedNumber']),
    searchQueryString () { return this.$route.query.search }
  },
  methods: {
    ...mapActions(['fetchUserData', 'fetchAllLocalNews', 'fetchConfirmedLocalNews', 'searchLocalNews']),
    ...mapMutations(['clearLocalNews']),
    ...
    async loadFirstNews (query = '', confirmed = false) {
      ...
      if (query === '') {
        // NOT confirmed
        if (!confirmed) {
          await this.fetchAllLocalNews(data).then(() => {
            this.firstNewsLoading = false
            if (this.localNewsFetchedNumber < this.articlesPerPage) this.nextPageIsEmpty = true
            if (this.localNewsFetchedNumber === 0) this.noResults = true
          })

        // CONFIRMED
        } else {
          await this.fetchConfirmedLocalNews(data).then(() => {
            this.firstNewsLoading = false
            if (this.localNewsFetchedNumber < this.articlesPerPage) this.nextPageIsEmpty = true
            if (this.localNewsFetchedNumber === 0) this.noConfirmedResults = true
          })
        }
      ...
      // SEARCH
        ...
        await this.searchLocalNews(data).then(() => {
          this.firstNewsLoading = false
          if (this.localNewsFetchedNumber < this.articlesPerPage) this.nextPageIsEmpty = true
          if (this.localNewsFetchedNumber === 0) this.noSearchResults = true
        })
      }
    },
    ...
    async searchSubmit () {
      ...
      this.$router.push({ name: 'local-news', query: { search: this.searchQuery } })
